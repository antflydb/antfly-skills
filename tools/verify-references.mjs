...
const forbidden = [
  ...
  { pattern: /return_level/, message: 'hierarchy has no return_level field — grouping levels are "source" and "unit"' },
  { pattern: /"level"\s*:\s*"chunk"|level: "chunk"/, message: 'hierarchy levels are "source" and "unit"; there is no "chunk" level' },
];
...
// ---------------------------------------------------------------- links
// A skill or doc pointing at a repo-relative .md path that doesn't exist.
{
  const pathLike = /`((?:references|skills|guides|schemas)\/[A-Za-z0-9_./-]+\.(?:md|ya?ml|json))`/g;
  for (const file of files) {
    const rel = relative(ROOT, file);
    if (/^PLAN-/.test(rel)) continue; // plan docs propose files that do not exist yet
    const lines = readFileSync(file, "utf8").split("\n");
    lines.forEach((line, i) => {
      for (const m of line.matchAll(pathLike)) {
        const fromRoot = join(ROOT, m[1]);
        const fromFile = join(file, "..", m[1]);
        if (!existsSync(fromRoot) && !existsSync(fromFile)) {
          problems.push(`${rel}:${i + 1}: broken path (checked repo-root and file-relative): ${m[1]}`);
        }
      }
    });
  }
